...
const Mint = () => {
  ...
  const [{ SAMB, WETH }, setCoins] = useState({ SAMB: {}, WETH: {} });
  useEffect(() => {
    if (tokenList) {
      const SAMB = tokenList.find((token) => token.symbol === 'SAMB');
  ...
  const mintSAMBinETH = async () => {
    ...
    await contract.increaseAllowance(ethContractAddress, amount);
  };
